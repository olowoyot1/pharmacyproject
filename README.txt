OAATZ Zoho Books Landing Page
=============================

Files:
- index.html: complete responsive landing page, no external dependencies.

Open index.html in any modern browser to preview.

Before production:
1. Connect the demo form to OAATZ's preferred CRM/email endpoint.
2. Replace any illustrative dashboard figures with live/approved figures.
3. Confirm final Zoho pricing before publishing.
4. Add the official OAATZ logo asset if desired.
